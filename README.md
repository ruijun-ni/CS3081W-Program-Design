...
We build a 3D map of UMN. There are robots and drone entities that exist on this map. The drone will appear at its initialized position when you build the project. Then you can schedule trips by setting up the starting position and destination for the robot. The drone will finish scheduled trips one by one. At first, the drone will check if its current energy can support the trip. If it is, the drone will pick up the robot and send it to the destination. Otherwise, the drone will first go to the energy station to recharge. Then it will continue the trip(go to pick up the robot and send it to the destination). The project will show the drone's current energy on the screen. We will have data.csv in the root directory of this project.
