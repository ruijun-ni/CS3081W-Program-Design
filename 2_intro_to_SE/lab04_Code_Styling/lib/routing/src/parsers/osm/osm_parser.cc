...
template <class T, class V>
unordered_map<T, bool>* where_equal(
  const unordered_map<T, int>* mapping,
  V equal_to
) {
...
class GraphUtils {
 public:
      static unordered_map<string, int>* ConnectedComponents(
        const IGraph* graph);
      static OSMGraph* Filter(
...
OSMGraph* GraphUtils::Filter(
  const IGraph* original,
  const unordered_map<string, bool>* filter
) {
    // NOTE: does not preserve the type of the graph that was input,
    // always gives back an OSMGraph
...
OSMGraph* GraphUtils::FilterToLargestConnectedComponent(
  const IGraph* original
    ...
    unordered_map<int, int>* cc_sizes =
      count_value_occurrences(cc_mapping);
...
OSMGraph* OsmParser::LoadGraphFromFile(string filename, bool debug) {
  ...
  // sanity check, make sure the document loaded,
  // and print something (anything) from the document
  #ifdef DEBUG
...
OSMGraph* OsmParser::read_nodes(pugi::xml_document* doc, bool debug) {
    ...
    for (way_node = parent_of_nodes.child("node");
      way_node != nullptr;
      way_node = way_node.next_sibling("node")) {
      if (!way_node.attribute("id")) {
        std::cerr
          << "Improperly formed node missing id. Continuing."
          << std::endl;
        continue;
      }
      if (!way_node.attribute("lat")) {
        std::cerr
          << "Improperly formed node missing lat. ID: "
      ...
      if (!way_node.attribute("lon")) {
        std::cerr
          << "Improperly formed node missing lon. ID: "
          << way_node.attribute("ref");
        std::cerr << ". Continuing" << std::endl;
        continue;
      }
      ...
      if (graph->Contains(id)) {
        std::cerr
          << "Attempted to add duplicate node. ID: "
          << way_node.attribute("ref");
        std::cerr << ". Continuing" << std::endl;
      }
...
void OsmParser::read_adjacencies_to(
  OSMGraph* graph, pugi::xml_document* doc,
  bool debug
) {
...
std::unordered_map<std::string, std::set<std::string>>
    ...
    bool debug) {
  // create a parent of all (interesting) nodes,
  // then get the first node the the "way" name
  pugi::xml_node parent_of_nodes = doc->first_child();
  ...
  while (way_node != NULL) {
    for (pugi::xml_node tag_node = way_node.child("tag");
         tag_node != nullptr;
  ...
  // get all the string ids of nodes that are a part of highway ways,
  // we don't care about the order and don't want repeats,
  // so we use a unordered set
  std::unordered_set<std::string> highway_node_ids =
    std::unordered_set<std::string>();
  // for every highway way node, any child named "nd" is a node
  // that is part of the way, so add it to the highway node ids set
  ...
  // we want an adjacency list that maps from a node's id
  // to a list of nodes it is connected to
  ...
  int num_edges = 0;
  // for every way node, adjacent "nd" nodes are connected,
  // iterate through and connect
