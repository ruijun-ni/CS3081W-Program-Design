...
const IGraphNode* GraphBase::NearestNode(
...
const std::vector< std::vector<float> > GraphBase::GetPath(
    std::vector<float> src, std::vector<float> dest,
    const RoutingStrategy& pathing
) const {
    using namespace std;
    ...
    vector<string> string_path = pathing.GetPath(
        this,
        start_node->GetName(),
        end_node->GetName());
