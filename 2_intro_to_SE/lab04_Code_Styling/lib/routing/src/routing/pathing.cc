...
bool compareCandidatePaths(
    const CandidatePath* path1,
            ...
            for (IGraphNode* next : next_steps) {
                ...
                possible_paths.push(
                    new CandidatePath(
                        ...
                        candidate->distance + cost->Calculate(
                            path_end_node->GetPosition(),
                            next->GetPosition()),
                        heuristic->Calculate(
                            next->GetPosition(),
...
std::vector<std::string> DepthFirstSearch::GetPath(
    const IGraph* graph, const std::string& from,
    const std::string& to
) const {
    unordered_set<string> visited;  // don't check nodes we've already visited
    queue<CandidatePath*> possible_paths;
    // queue of all paths we're considering in BFS
